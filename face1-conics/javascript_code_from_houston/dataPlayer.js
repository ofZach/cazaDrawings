...
class dataPlayer {
	...
	update() {
		...
		if (this.liveData == false || typeof this.frameData === "undefined") {
			this.frameData = this.dataObject[this.frame];
		} else {
			if (typeof window.frameData !== "undefined") {
				if (typeof window.frameData.frameData !== "undefined" &&
					window.frameData.frameData !== null && typeof window.dataSmoothing !== "undefined") {
					if (window.frameData.frameData.length == 68) {

						var smooth = window.dataSmoothing;
						var one_m_smooth = 1.0 - smooth;

						for (var i = 0; i < 68; i++) {
							this.frameData[i][0] = smooth * this.frameData[i][0] + (one_m_smooth) * window.frameData.frameData[i][0];
							this.frameData[i][1] = smooth * this.frameData[i][1] + (one_m_smooth) * window.frameData.frameData[i][1];

						}
						//frameData = window.frameData.frameData;
						this.viewScale = 1;
						this.offset.x = 0;
						this.offset.y = 0;
					}
				}
				//console.log("adding! " + frameData);
			}
		}
		...
		for (var i = 0; i < 68; i++) {
			...
			this.frameAnalysis['points'][i].x = this.frameData[i][0] * this.viewScale + this.offset.x;
			this.frameAnalysis['points'][i].y = this.frameData[i][1] * this.viewScale + this.offset.y;
